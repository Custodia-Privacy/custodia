...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res === undefined) {
        setError("Sign-in did not complete. Refresh the page and try again.");
        return;
      }

      // Auth.js returns HTTP 200 JSON with `error` in the redirect URL for failed credentials.
      if (res.error) {
        if (res.error === "CredentialsSignin") {
          setError("Invalid email or password.");
        } else if (res.error === "MissingCSRF") {
          setError(
            "Security check failed (CSRF). Use the same site you opened in the address bar for the whole session — do not mix http://127.0.0.1 and http://localhost — then refresh this page and try again.",
          );
        } else {
          setError(`Unable to sign in (${res.error}). Please try again.`);
        }
        return;
      }
      ...
      setError(
        "Something went wrong while signing in (often a bad response from the server). Check the terminal for errors and confirm DATABASE_URL and NEXTAUTH_SECRET are set.",
      );
    } finally {
      setLoading(false);
    }
