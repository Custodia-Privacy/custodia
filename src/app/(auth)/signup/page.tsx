...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const canSignInNow = Boolean(data.canSignInNow);
      if (!canSignInNow) {
        setInfo(
          "Check your email for a verification link. After you verify, you can log in with this password.",
        );
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-lg dark:border-slate-800 dark:bg-slate-950">
      ...
      {info && (
        <div className="mb-4 rounded-lg bg-sky-50 px-4 py-2 text-sm text-sky-800 dark:bg-sky-950/40 dark:text-sky-200">
          {info}
        </div>
      )}
