...
export const dashboardRouter = createRouter({
  overview: orgProcedure.query(async ({ ctx }) => {
    ...
    return cached(`dash:${orgId}`, 60, async () => {
      ...
      const [scans, trackerCount, openDsars, policies, alerts, recentScans, dsarActivities, agentRuns] = await Promise.all([
        siteIds.length > 0
          ? ctx.db.scan.findMany({
              where: { siteId: { in: siteIds }, status: "completed" },
              orderBy: { completedAt: "desc" },
              take: Math.max(siteIds.length * 2, 10),
              select: {
                siteId: true,
